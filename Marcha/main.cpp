...
*/
static std::vector<Rhoban::IKWalkOutputs> runWalk(
	...
	double& time)
{
	...
	struct std::vector<Rhoban::IKWalkOutputs> outputs;
	struct Rhoban::IKWalkOutputs output;
	...
	for (double t = 0.0; t <= timeLength; t += 1.0 / engineFrequency) {
		...
		bool success = Rhoban::IKWalk::walk(
			...
			output); //Result target position (updated)
		if (!success) {
			...
			std::cout << output.left_hip_yaw << " ";
			std::cout << output.left_hip_pitch << " ";
			std::cout << output.left_hip_roll << " ";
			std::cout << output.left_knee << " ";
			std::cout << output.left_ankle_pitch << " ";
			std::cout << output.left_ankle_roll << " ";
			std::cout << output.right_hip_yaw << " ";
			std::cout << output.right_hip_pitch << " ";
			std::cout << output.right_hip_roll << " ";
			std::cout << output.right_knee << " ";
			std::cout << output.right_ankle_pitch << " ";
			std::cout << output.right_ankle_roll << " ";
			std::cout << std::endl;
...
int main()
{
	...
	*/
	...
	for (auto salida : runWalk(params, 2.0, phase, time))
	{
		std::cout << "a";
	}
